...
//#define USEDMA
//use ss_semaphore library instead of scratch_sempahore library -> need mparm hw support
#define USEINTERRUPT

//enable queue instantiation which has data buffer in shared memory
//#define USE_SHARED_BUFFER
//use memcpy function instead of for cicle
//#define MEMCPY 
...
#define QUEUE_BASE (CORESLAVE_BASE+SCRATCH_SIZE_PLATFORM)
...
#else
void scratch_queue_autoinit_system(unsigned int scra_start=0,unsigned
int scra_size=0);
#endif
//void scratch_queue_autoinit_system(void);


SCRATCH_QUEUE_PRODUCER* scratch_queue_autoinit_producer(int core_consumer_id, int queue_id, int n_items, 
       int token_size, int bufferzone=0);
...
inline char* scratch_queue_read_dma(register SCRATCH_QUEUE_CONSUMER *queue_p){
  scratch_queue_read_dma(queue_p,queue_p->local_buffer);
  #ifdef DEBUGSWI
    SHOW_DEBUG("Read from local buffer");
    SHOW_DEBUG_INT(queue_p->local_buffer);
  #endif
