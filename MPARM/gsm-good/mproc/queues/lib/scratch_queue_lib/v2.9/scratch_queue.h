...
inline void scratch_queue_write(register SCRATCH_QUEUE_PRODUCER *queue_p,register  char *data){
  ...
  #else
  register int offset=queue_p->index_write*queue_p->message_size;
  for(register int i=0; i<(queue_p->message_size/4); i++)
  ((int*)queue_p->fifo_p)[offset+i]=((int*)data)[i];
  #endif
...
inline void scratch_queue_read(register SCRATCH_QUEUE_CONSUMER *queue_p,
  ...
  /*read message from queue*/
   ...
   #else 
    register int offset=queue_p->index_read*queue_p->message_size;
    for(register int i=0; i<(queue_p->message_size/4); i++)
      ((int*)buffer)[i]=((int*)queue_p->fifo_p)[offset+i];
   #endif
