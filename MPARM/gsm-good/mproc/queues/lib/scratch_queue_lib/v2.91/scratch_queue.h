...
inline void scratch_queue_write(register SCRATCH_QUEUE_PRODUCER *queue_p,register  char *data){
  ...
  #else
    register int* address=(int*)&(queue_p->fifo_p[queue_p->index_write*queue_p->message_size]);
    for(register unsigned int i=0; i<(queue_p->message_size/sizeof(int)); i++) address[i]=((int*)data)[i];    
  #endif
...
inline void scratch_queue_read(register SCRATCH_QUEUE_CONSUMER *queue_p,register char *buffer){
  ...
  /*read message from queue*/
   ...
   #else 
     register int* address=(int*)&(queue_p->fifo_p[queue_p->index_read*queue_p->message_size]);
     for(register unsigned int i=0; i<(queue_p->message_size/sizeof(int)); i++) ((int*)buffer)[i]=address[i];
   #endif
