...
* v 2.4 - General fixing & refing (added some optimization like register)
* v 2.3 - Added DEBUGSWII macro for interrupt handling debugging
* v 2.1 - DEBUG PRINT RE-ENGEENERING
* v 2.1 - interrupt disabling sequence changed (shrinked int-insensitive code pieces)
* v 2.0 - DF BUG localized only in directive failed with status call, code removed by comment
* v 2.0 - BUG on enabling DF define.. maybe dir_failed macro bug... keep it down! (but they can't last :-))
...
// How system works? (it works? :-)) (maybe it's weather dependent...) (other bands play, Manowar kills!)
...
#include <bsp.h> //for SHOW_DEBUG macros & rtems functions

#define IRQ_PIN int22

//#define DEBUG0 //prints on serial ports
//#define DEBUGSWI //print via swi calls
//#define DEBUGSWII //print via swi calls on iterrupt suspensions/enabling
//#define DF //invokes directive failed after rtems calls
//#define ESC //Explicit status check after rtems calls

#ifdef DF
  #include "tmacros.h" //for directive failed functions -> include bsp.h another time!
#endif
...
#include "appsupport.h"  //For swi debug
...
void interrupt_handler(){
  ...
  for(int i=0;i<lnumber;i++) if(scratch_table[i]!=0){
    register int value=queue_table[i];
      if(value>0) {
	...
	#endif
	#ifdef DF
	  //fatal_directive_status_with_level(status, RTEMS_SUCCESSFUL,"rtems_task_resume",1); //removed due to bug see above
	  SHOW_DEBUG("DF with success");
	#endif
      }
    }
  #ifdef DEBUGSWII
...
int ss_sem_system_init(unsigned int queue_address,unsigned int scratch_address, int num){
  ...
  static rtems_irq_connect_data dma_int_data = { IRQ_PIN, interrupt_handler, son, soff, sison, 0, 0};
...
void ss_sem_wait(SS_SEMAPHORE my_semaphore){
  ...
  if(value<=0){
    ...
    #ifdef DEBUGSWII
      SHOW_DEBUG("WAIT: suspending task");
    #endif
...
unsigned int ss_sem_wait_read(SS_SEMAPHORE my_semaphore){
  ...
  register rtems_interrupt_level level;
  #ifdef DEBUGSWII
    SHOW_DEBUG("WAIT_READ: disabling interrupts");
  #endif
  rtems_interrupt_disable(level);
  #ifdef DEBUG0
    printf("ss_sem_wait: %x\n", my_semaphore);
  ...
  if(value<=0){
    ...
    scratch_table[displacement]=my_id;
    #ifdef DEBUGSWII
      SHOW_DEBUG("WAIT_READ: suspending task");
    #endif
  ...
  #ifdef DEBUGSWII
    SHOW_DEBUG("WAIT_READ: enabling interrupts");
  #endif
  rtems_interrupt_enable(level);
  return value;
}
